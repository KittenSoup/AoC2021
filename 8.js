...
const dayeight = () => {
  ...
  const buildMapping = (input) => {
    ...
    inputArr.map((maybeNine) => {
      if (maybeNine.length === 6) {
        if (reverseMap[4].split("").every(letter => maybeNine.includes(letter))) {
          mapping[maybeNine] = 9
    ...
    inputArr.map((sixOrZero) => {
      if (sixOrZero.length === 6) {
        if (reverseMap[1].split("").every(letter => sixOrZero.includes(letter) && sixOrZero !== reverseMap[9])) {
          mapping[sixOrZero] = 0
    ...
    inputArr.map((threeFiveOrTwo) => {
      ...
      const fourMinusOne = reverseMap[4].replace(reverseMap[1][0], "").replace(reverseMap[1][1], "")
      if (threeFiveOrTwo.length === 5) {
        if (reverseMap[1].split("").every(letter => threeFiveOrTwo.includes(letter))) {
          mapping[threeFiveOrTwo] = 3
          reverseMap[3] = threeFiveOrTwo
        }
        else if (fourMinusOne.split("").every(letter => threeFiveOrTwo.includes(letter))) {
  ...
  try {
    ...
    const answerPt2 = data.map((line) => {
      const output = getOutput(line)
      const input = getInput(line)
      const mapping = buildMapping(input)
      const number = output.split(" ").map((codedNumber) => findNumber(codedNumber, mapping)).join("")
      return number
    }).map(i => parseInt(i, 10)).reduce((p, v) => p+v)

    const answer = data.map(getOutput).map(countUniqueDigits).reduce((p, v) => p+v)
